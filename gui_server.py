...
def api_stop():
    global stop_requested
    stop_requested = True
    # Tell the backend engine to abort the running generation
    try:
    ...
    def stream():
        ...
        try:
            ...
            for chunk in resp.iter_content(chunk_size=None, decode_unicode=True):
                if stop_requested:
                    # Close the stream; engine-side abort already triggered by /api/stop
                    resp.close()
...
if __name__ == "__main__":
    ...
    print("=" * 45)
    app.run(host="127.0.0.1", port=8080, threaded=True, debug=False, use_reloader=False)
